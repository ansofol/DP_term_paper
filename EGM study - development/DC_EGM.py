...
def value_of_choice_working(m,c,type,t,sol,par):
    ...
    for i_eps in range(par.neps): 
        try: 
            V_next_work += tools.interp_linear_1d_scalar(sol.m[type,t+1,1,t+1,:,i_eps], sol.V[type,t+1,1,t+1,:,i_eps], m_next)*par.eps_w[i_eps]
        except: 
            V_next_work += tools.interp_linear_1d(sol.m[type,t+1,1,t+1,:,i_eps], sol.V[type,t+1,1,t+1,:,i_eps], m_next)*par.eps_w[i_eps]
