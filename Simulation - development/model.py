...
class Model():
    ...
    def setup(self):
        ...
        par.beta = 0.5 # Time preference
        par.vartheta = 0.0415 # Disutility parameter of work
    ...
    def set_grids(self):
        ...
        par.lambda_vec = - tools.nonlinspace(-0.15, -0.05 , par.Smax+1, 1.05)
        ...
        sim.c = np.zeros(shape_sim) + np.nan
        sim.S = np.zeros(shape_sim) 
        sim.ell = np.zeros(shape_sim) + np.nan
        sim.a = np.zeros(shape_sim) + np.nan
